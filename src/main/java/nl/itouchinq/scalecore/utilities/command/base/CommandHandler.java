...
import nl.itouchinq.scalecore.utilities.command.annotation.*;
import nl.itouchinq.scalecore.utilities.command.annotation.Optional;
import nl.itouchinq.scalecore.utilities.command.base.components.CommandData;
import nl.itouchinq.scalecore.utilities.command.exceptions.FEException;
...
public final class CommandHandler extends Command {
    ...
    void addSubCommands(final CommandBase command) {
        ...
        for (final Method method : command.getClass().getDeclaredMethods()) {
            ...
            if (method.getParameterCount() == 0)
                throw new FEException("Method " + method.getName() + " in class " + command.getClass().getName() + " - needs to have Parameters!");
            ...
            if (!CommandSender.class.isAssignableFrom(method.getParameterTypes()[0]))
                throw new FEException("Method " + method.getName() + " in class " + command.getClass().getName() + " - first parameter needs to be a CommandSender, Player, or ConsoleCommandSender!");
    ...
    private void checkRegisteredParams(final Method method, final CommandBase command, final CommandData subCommand) {
        ...
        for (int i = 1; i < method.getParameterTypes().length; i++) {
            ...
            if (clss.equals(String[].class) && i != method.getParameterTypes().length - 1) {
                throw new FEException("Method " + method.getName() + " in class " + command.getClass().getName() + " 'String[] args' have to be the last parameter if wants to be used!");
            }

            if (!this.parameterHandler.isRegisteredType(clss)) {
                throw new FEException("Method " + method.getName() + " in class " + command.getClass().getName() + " contains unregistered parameter types!");
            }
    ...
    private void checkParamCompletion(final Method method, final CommandBase command, final CommandData subCommand) {
        ...
        for (int i = 0; i < method.getParameters().length; i++) {
            ...
            if (i == 0 && parameter.isAnnotationPresent(Completion.class))
                throw new FEException("Method " + method.getName() + " in class " + command.getClass().getName() + " - First parameter of a command method cannot have Completion/Values annotation!");
            ...
            if (values.length != 1)
                throw new FEException("Method " + method.getName() + " in class " + command.getClass().getName() + " - Parameter completion can only have one value!");

            if (!values[0].startsWith("#"))
                throw new FEException("Method " + method.getName() + " in class " + command.getClass().getName() + " - The completion ID must start with #!");

            if (completionHandler.isNotRegistered(values[0]))
                throw new FEException("Method " + method.getName() + " in class " + command.getClass().getName() + " - Unregistered completion ID '" + values[0] + "'!");
    ...
    private void checkMethodCompletion(final Method method, final CommandBase command, final CommandData subCommand) {
        ...
        for (int i = 0; i < completionValues.length; i++) {
            ...
            if (completionHandler.isNotRegistered(id))
                throw new FEException("Method " + method.getName() + " in class " + command.getClass().getName() + " - Unregistered completion ID'" + id + "'!");
    ...
    private void checkOptionalParam(final Method method, final CommandBase command, final CommandData subCommand) {
        ...
        for (int i = 0; i < method.getParameters().length; i++) {
            ...
            if (i != method.getParameters().length - 1 && parameter.isAnnotationPresent(Optional.class))
                throw new FEException("Method " + method.getName() + " in class " + command.getClass().getName() + " - Optional parameters can only be used as the last parameter of a method!");
    ...
    private boolean wrongUsage(final CommandSender sender, final CommandData subCommand) {
        ...
        if (!wrongMessage.startsWith("#") || !messageHandler.hasId(wrongMessage)) {
            messageHandler.sendMessage("cmd.wrong.usage", sender);
            sender.sendMessage(CC.translate(subCommand.getWrongUsage()));
            return true;
        }
